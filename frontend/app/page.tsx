...
export default function HomePage() {
    ...
    useEffect(() => {
        ...
        const fromYMD = `${from.getFullYear()}-${pad2(from.getMonth() + 1)}-${pad2(from.getDate())}`;
        const toYMD = `${to.getFullYear()}-${pad2(to.getMonth() + 1)}-${pad2(to.getDate())}`;
        (async () => {
            try {
                const m = await api.releaseDays(fromYMD, toYMD) as Markers; // ожидается эндпоинт /api/release-days
                setMarkers(m || {});
            } catch {
                setMarkers({});
            }
    ...
    const createRelease = async (payload: ReleasePayload) => {
        ...
        setMode("view");
        await load();
        setVisibleMonth(new Date(visibleMonth)); // <-- триггерим перезагрузку маркеров месяца
    };
    const updateRelease = async (payload: ReleasePayload) => {
        ...
        setMode("view");
        await load();
        setVisibleMonth(new Date(visibleMonth)); // <-- обновляем точки
    };
    ...
    function DayContent(props: DayContentProps) {
        ...
        const sts = markers[key] || [];
        return (
            <div className="relative flex items-center justify-center">
                <span>{d.getDate()}</span>
                {sts.length > 0 && (
                    <span className="pointer-events-none absolute -bottom-0.5 left-1/2 -translate-x-1/2 flex gap-0.5">
          {sts.includes("failed")  && <span className="w-1 h-1 rounded-full bg-red-500"   />}
                        {sts.includes("success") && <span className="w-1 h-1 rounded-full bg-green-500" />}
                        {sts.includes("planned") && <span className="w-1 h-1 rounded-full bg-blue-500"  />}
        </span>
                )}
            </div>
        );
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div className="md:col-span-5 card">
                ...
                <DayPicker
                    ...
                    components={{ DayContent }}   // <-- добавили точки
                />
                <div className="mt-4">
                    ...
                    <div className="space-y-4">
                        <div className="card">
                            ...
                            <div className="text-sm text-neutral-500">
                                {new Date(active.date).toLocaleString()} · <span className="badge border-neutral-300">{active.status}</span>
                            </div>
